...
module.exports = class CachedInputFileSystem {
	constructor(fileSystem, duration) {
		...
		this._readRealpathStorage = new Storage(duration);
		...
		//fs.realpath.native was added since node v9.2.0
		if(this.fileSystem.realpath && this.fileSystem.realpath.native) {
			this._readRealpath = this.fileSystem.realpath.native.bind(this.fileSystem);
		}
		if(!this._readRealpath) this.readRealpath = null;

		if(this.fileSystem.realpathSync && this.fileSystem.realpathSync.native) {
			this._readRealpathSync = this.fileSystem.realpathSync.native.bind(this.fileSystem);
		}
		if(!this._readRealpathSync) this.readRealpathSync = null;
	}
	...
	readRealpath(path, callback) {
		return this._readRealpathStorage.provide(path, this._readRealpath, callback);
	}
	...
	readRealpathSync(path) {
		return this._readRealpathStorage.provideSync(path, this._readRealpathSync);
	}
